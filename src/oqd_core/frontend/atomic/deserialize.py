...
class Deserialize(ConversionRule):
...
def deserialize_atomic(source):
    
    try:
        circuit = AtomicCircuit.model_validate_json(source)
    except Exception as e:
        raise TypeError(f"Expected JSON input: {e}")
    
    return Post(Deserialize())(circuit)

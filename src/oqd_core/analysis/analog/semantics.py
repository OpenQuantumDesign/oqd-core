    ...
    def infer_type(self, expr, env: TypeEnv) -> TLatticeValue:
        ...
        if isinstance(expr, TERMINAL_NODE_TYPES):
            ...
            if isinstance(expr, Bool):
                return TBool
            if isinstance(
                expr, (PauliI, PauliX, PauliY, PauliZ, Creation, Annihilation, Identity)
            ):
                return TOp
        ...
        if isinstance(expr, OperatorMul):
            ...
            if out is None:
                raise AnalogTypeError(
                    f"{type(expr).__name__} expects operator or scalar, got {type_name(t1)}, {type_name(t2)}"
                )
            return out
        ...
        if isinstance(expr, (Initialize, Measure)):
            ...
            if isinstance(t, TList):
                if not self.value_lattice.leq(t.elem, TTargetRef):
                    raise AnalogTypeError(
                        f"{type(expr).__name__} expects Quantum targets, got {type_name(t)}"
                    )
        ...
        if isinstance(expr, Evolve):
            ...
            if isinstance(target_t, TList):
                if not self.value_lattice.leq(target_t.elem, TTargetRef):
                    raise AnalogTypeError(
                        f"{type(expr).__name__} expects Quantum targets, got {type_name(target_t)}"
                    )
            ...
            if not self.value_lattice.leq(hamiltonian_t, TOp):
                raise AnalogTypeError(
                    f"{type(expr).__name__} expects operator hamiltonian, got {type_name(hamiltonian_t)}"
                )

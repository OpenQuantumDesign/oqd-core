...
from oqd_core.analysis.utils import alias_types
...
class AtomicSemantics:
    ...
    def infer_type(self, expr: type, env: TypeEnv) -> TLatticeValue:
        ...
        if sig is not None:
            ...
            if not self.value_lattice.leq(t1, lreq) or not self.value_lattice.leq(
                t2, rreq
            ):
                raise AtomicTypeError(
                    f"{type(expr).__name__} got {type_name(t1)}, {type_name(t2)} expected {type_name(lreq)}, {type_name(rreq)}"
                )
            return out
        ...
        if isinstance(expr, MathFunc):
            if expr.func in MATH_FUNCS:
                ...
                if not self.value_lattice.leq(t, TScalar):
                    raise AtomicTypeError(
                        f"{expr.func} expects scalar, got {type_name(t)}"
                    )
                return TScalar
            ...
            if expr.func == "atan2":
                ...
                if not self.value_lattice.leq(
                    t1, TScalar
                ) or not self.value_lattice.leq(t2, TScalar):
                    raise AtomicTypeError(
                        f"{expr.func} expects scalar, got {type_name(t1)}, {type_name(t2)}"
                    )
                return TScalar
        ...
        if isinstance(expr, (BoolEq, BoolNotEq)):
            ...
            if t1 not in (TBool, TScalar) or t2 not in (TBool, TScalar):
                raise AtomicTypeError(
                    f"{type(expr).__name__} expects bool or scalar, got {type_name(t1)}, {type_name(t2)}"
                )
            if t1 is not t2:
                raise AtomicTypeError(
                    f"{type(expr).__name__}: got {type_name(t1)} vs {type_name(t2)}"
                )
            return TBool
        ...
        if isinstance(expr, BoolNot):
            ...
            if not self.value_lattice.leq(t, TBool):
                raise AtomicTypeError(
                    f"{type(expr).__name__} expects bool, got {type_name(t)}"
                )
            return TBool
        ...
        if isinstance(expr, Beam):
            if isinstance(expr, Beam):
                ...
                if not self.value_lattice.leq(freq_t, TScalar):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects scalar frequency, got {type_name(freq_t)}"
                    )

                rabi_t = self.infer_type(expr.rabi, env)
                if not self.value_lattice.leq(rabi_t, TScalar):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects scalar rabi, got {type_name(rabi_t)}"
                    )

                phase_t = self.infer_type(expr.phase, env)
                if not self.value_lattice.leq(phase_t, TScalar):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects scalar phase, got {type_name(phase_t)}"
                    )
                ...
                if isinstance(pol_t, TList):
                    if not self.value_lattice.leq(pol_t.elem, TScalar):
                        raise AtomicTypeError(
                            f"{type(expr).__name__} expects scalar polarization components, got {type_name(pol_t)}"
                        )
                elif not self.value_lattice.leq(pol_t, TScalar):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects scalar polarization, got {type_name(pol_t)}"
                    )
                ...
                if isinstance(wave_t, TList):
                    if not self.value_lattice.leq(wave_t.elem, TScalar):
                        raise AtomicTypeError(
                            f"{type(expr).__name__} expects scalar wavevector components, got {type_name(wave_t)}"
                        )
                elif not self.value_lattice.leq(wave_t, TScalar):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects scalar wavevector, got {type_name(wave_t)}"
                    )
        ...
        if isinstance(expr, Pulse):
            ...
            if not self.value_lattice.leq(duration_t, TScalar):
                raise AtomicTypeError(
                    f"{type(expr).__name__} expects scalar duration, got {type_name(duration_t)}"
                )
            ...
            if isinstance(target_t, TList):
                if not self.value_lattice.leq(target_t.elem, TTargetRef):
                    raise AtomicTypeError(
                        f"{type(expr).__name__} expects ion targets, got {type_name(target_t)}"
                    )
            ...
            if not self.value_lattice.leq(beam_t, TBeam):
                raise AtomicTypeError(
                    f"{type(expr).__name__} expects beam expression, got {type_name(beam_t)}"
                )
            measured_t = self.infer_type(expr.measured, env)
            if not self.value_lattice.leq(measured_t, TBool):
                raise AtomicTypeError(
                    f"{type(expr).__name__} expects bool measured flag, got {type_name(measured_t)}"
                )
